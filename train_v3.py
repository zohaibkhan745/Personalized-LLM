"""
train_v3.py — Fine-tune Meta-Llama-3-8B-Instruct with QLoRA on a single RTX 4090.

Changes vs train_v2.py (conservative LoRA but increased epochs for testing):
  1. LoRA target_modules: attention-only ["q_proj","k_proj","v_proj","o_proj"] (same as v2)
  2. LoRA rank / alpha:   r=8, lora_alpha=16  (same as v2)
  3. num_train_epochs:    15                  (was 2 in v2)
  4. output_dir:          ./llama3-math-tutor-adapter-v3

Everything else is identical to train_v2.py.
...
OUTPUT_DIR = "./llama3-math-tutor-adapter-v3"   # ← CHANGE 4: separate output dir
...
# ---------------------------------------------------------------------------
# 6. LoRA  (CHANGES 1 & 2: attention-only modules, r=8, lora_alpha=16)
# ---------------------------------------------------------------------------
def apply_lora(model):
    ...
    lora_config = LoraConfig(
        r=8,                        # ← CHANGE 2: was 16
        lora_alpha=16,              # ← CHANGE 2: was 32
        lora_dropout=0.05,
        bias="none",
        task_type="CAUSAL_LM",
        target_modules=[            # ← CHANGE 1: attention-only (removed gate_proj, up_proj, down_proj)
...
def get_training_args() -> SFTConfig:
    ...
    return SFTConfig(
        ...
        num_train_epochs=15,                        # ← CHANGE 3: was 2
